...
function UserConfirmationForm () {
    ...
    if (!lastModal)
    return;
    if (lastModal.type !== MODALS.USER_CONFIRMATION)
    return;
    if (!lastModal.message)
    return;

    const {DeleteWorkspace, DeleteBoard, DeleteTask} = WorkspaceStoreSelectors();
    ...
    const handleUserAccepted = () => {
        if (lastModal.modalStatus) {
            StatusToFunction[lastModal.modalStatus](lastModal.workspaceId, lastModal.boardId, lastModal.taskId);
        }
    ...
    return (
        <p className="modal-default">
            ...
            <button onClick={handleUserAccepted} className="save-button"><span className="white">Yes</span></button>
            <button onClick={() => ModalClosed()} className="save-button"><span className="white">No</span></button>
        </p>
    );
}
