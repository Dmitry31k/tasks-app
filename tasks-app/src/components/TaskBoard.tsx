...
function TaskBoard({board, onTaskModalOpened, onBoardDeleted, onTaskDeleted}: Props) {
    ...
    return (
        <>
            ...
            <ul>
                {tasks.map(task => <li key={task.taskId}>{task.title}
                    <button onClick={() => handleTaskDeleted(task.taskId)}>Delete task</button>
                </li>)}
            </ul>
