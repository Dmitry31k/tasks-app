...
function App() {
    const {workspaces, GetActiveWorkspaceIndex, SwitchToNextWorkspace, SwitchToPrevWorkspace} = WorkspaceStoreSelectors();
    ...
    return (
        <>
            ...
            <button className="default-button"
                onClick={() => {ModalOpened({type: MODALS.CREATE_WORKSPACE})}}
            >
                <p className="white">Add new workspace</p>
            </button>
            {workspaces.length === 0 && <span>You don't have any active workspaces</span>}
            <Workspace 
                workspace={workspaces[GetActiveWorkspaceIndex()]}
            ></Workspace>
            <ModalManager/>
        </>
    )
